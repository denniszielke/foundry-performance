...
weather tool uses direct MCP by default; ``WEATHER_TOOL_MODE=toolbox`` routes it
through Foundry toolbox. Foundry pulls the image, runs it, and fronts the
invocations protocol. Records the agent name in ``.env``
...
def main(tag: str | None = None) -> None:
    ...
    save_env({
        "WEATHER_HOSTED_AGENT_INVOCATIONS_NAME": AGENT_NAME,
        "WEATHER_HOSTED_AGENT_INVOCATIONS_IMAGE": image,
        "WEATHER_HOSTED_AGENT_INVOCATIONS_TOOL_MODE": tool_mode,
    })
