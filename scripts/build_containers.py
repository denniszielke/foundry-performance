...
``WEATHER_HOSTED_AGENT_INVOCATIONS_IMAGE`` / ``WEATHER_CUSTOM_AGENT_IMAGE``)::
...
IMAGES = [
    ...
    ("weather-custom-agent", ROOT / "src" / "custom_agent" / "Dockerfile", "WEATHER_CUSTOM_AGENT_IMAGE"),
]


def main(tag: str | None = None) -> None:
    load_env()
    # Resolve once so all three images share the same tag for this build.
