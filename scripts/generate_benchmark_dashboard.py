...
def _load_run(path: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "session-mode": data.get("session-mode", "unknown"),
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .filters { display: grid; grid-template-columns: repeat(7, minmax(120px, 1fr)); gap: 12px; padding: 16px; margin-bottom: 16px; }
...
<body>
  <main class="shell" id="app">
    ...
    <section class="panel filters" aria-label="Dashboard filters">
      ...
      <label>Session mode<select id="session-mode"></select></label>
    ...
    <section class="panel table-panel">
      ...
      <div class="table-wrap"><table>
        <thead><tr>
          <th data-key="agent-type">Agent</th><th data-key="model-hosting">Hosting</th><th data-key="model-deployment">Model</th><th data-key="tool-mode">Tool mode</th><th data-key="session-mode">Session mode</th>
  ...
  <script>
    ...
    const filterIds = ["agent-type", "model-hosting", "model-deployment", "tool-mode", "session-mode", "protocol"];
    ...
    function normalizeRun(data, source) {
      ...
      return {source, datetime: data.datetime, "agent-type": data["agent-type"], "model-hosting": data["model-hosting"],
        "model-deployment": data["model-deployment"], "tool-mode": data["tool-mode"] || "unknown", "session-mode": data["session-mode"] || "unknown", iterations: data.iterations, query: data.query, "base-url": data["base-url"],
        quality: qualityFor(Array.isArray(data.turns) ? data.turns : [], data.results), results: data.results};
    }
    function rows() {
      return state.runs.flatMap((run) => run.results.map((row) => ({...row, "tool-mode": row["tool-mode"] || run["tool-mode"] || "unknown", "session-mode": row["session-mode"] || run["session-mode"] || "unknown", source: run.source, datetime: run.datetime, quality: run.quality})));
    }
    ...
    function renderTable(items) {
      ...
      byId("table-body").innerHTML = sorted.length ? sorted.map((row) => `<tr class="${row.quality.suspicious ? "flagged" : ""}" title="${escapeHtml(row.source)}"><td>${escapeHtml(row["agent-type"])}</td><td>${escapeHtml(row["model-hosting"])}</td><td>${escapeHtml(row["model-deployment"])}</td><td>${escapeHtml(row["tool-mode"])}</td><td>${escapeHtml(row["session-mode"])}</td><td>${escapeHtml(row.protocol)}</td><td>${escapeHtml(row.phase)}</td><td class="number">${number(row.n) ?? "--"}</td><td class="number ${number(row.err) ? "error" : ""}">${number(row.err) ?? "--"}</td><td class="number">${formatMs(row["mean-ms"])}</td><td class="number">${formatMs(row.p50)}</td><td class="number">${formatMs(row.p95)}</td><td class="number">${formatMs(row.ttfb)}</td><td>${new Date(row.datetime).toLocaleString()}</td></tr>`).join("") : `<tr><td colspan="14" class="empty">No measurements match the filters.</td></tr>`;
    }
    function render() {
